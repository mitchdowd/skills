---
name: breakdown
description: Break down a requirements document into individually executable tasks, ready for execution.
argument-hint: Provide requirements documentation to break into tasks
user-invocable: true
disable-model-invocation: false
---

Break a requirements document into individually implementable tasks.

### 1. Set Task Location

Check with the user where they would like the tasks to be created. Suggested options include:
 - Jira (recommended). If selected, confirm the Jira Space and Epic to create the tasks under. Recommend a newly-created Epic, but allow the user to specify an existing Epic. Verify that you have Jira access via either Atlassian's MCP or CLI. If you do not, inform the user and halt.
 - Github Issues. If selected, verify access via either MCP or CLI. If no access, inform the user and halt.
 - Other. Allow the user to specify where and how they would like the tasks created.
...
Then, for each approved task, create the task in the requested location. Each task should have:

- The task title (do not prefix with any sort of ID or code if entered into a system such as Jira or Github Issues, as these will be automatically generated by the system)
- A description of the work to be done
- A dot-point list of acceptance criteria
- A list of blockers

Jira note: In Jira you must specify blockers as linked Jira items, not as notes in the task description. The blocker should be created as a "blocks" link on the blocker itself (not on the "blocked" task).

Do *not* specify effort estimates, priority labels, assignees or dates on the tasks.

### 7. Output Result

Output a link to the resulting task breakdown to the user for them to enter to see the results.
